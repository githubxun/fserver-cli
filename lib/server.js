...
function Context(env, options){
    ...
    _assign(this.CONFIG.server, {server});
...
Context.prototype._defaultConfig = function(wp){
    var config = {};
    if(shell.test('-f', 'webpack.config.dev.js')){
        config.webpack = path.join(this.work_dirname, wp || 'webpack.config.dev');
    }
    else{
        config.webpack = path.join(this.config_dirname, 'webpack.config.dev');
    }
